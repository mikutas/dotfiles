...
function renovate
	if ls ./.envrc
		docker run --rm -it -v (pwd):/usr/src/app renovate/renovate:43.214.0 sh -c "curl -sfL https://direnv.net/install.sh | bash && direnv allow . && direnv exec . renovate $argv"
	else
		docker run --rm -it -v (pwd):/usr/src/app renovate/renovate:43.214.0 sh -c "renovate $argv"
	end
    rm -rf renovate/
end

function renovate-config-validator
	docker run --rm -it -v (pwd):/usr/src/app renovate/renovate:43.214.0 sh -c "renovate-config-validator"
end
