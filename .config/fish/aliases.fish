...
function renovate
	if ls ./.envrc
		docker run --rm -it -v (pwd):/usr/src/app renovate/renovate:39.233.3 sh -c "curl -sfL https://direnv.net/install.sh | bash && direnv allow . && direnv exec . renovate $argv"
	else
		docker run --rm -it -v (pwd):/usr/src/app renovate/renovate:39.233.3 sh -c "renovate $argv"
	end
    rm -rf renovate/
end
