...
function renovate
	if ls ./.envrc
		docker run --rm -it -v (pwd):/tmp renovate/renovate:37.213.0-slim sh -c "curl -sfL https://direnv.net/install.sh | bash && cd /tmp && direnv allow . && direnv exec . renovate $argv"
	else
		docker run --rm -it -v (pwd):/tmp renovate/renovate:37.213.0-slim sh -c "renovate $argv"
	end
    rm -rf renovate/
end

function renovate-config-validator
	docker run --rm -it -v (pwd):/tmp renovate/renovate:37.213.0-slim sh -c "cd /tmp && renovate-config-validator"
end
