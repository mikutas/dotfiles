...
# go required
function aqua-setup
	curl -sSfL https://raw.githubusercontent.com/aquaproj/aqua-installer/v3.0.1/aqua-installer | bash -s -- -v v2.37.2
end
...
function renovate
	if ls ./.envrc
		docker run --rm -it -v (pwd):/tmp renovate/renovate:38.142.4 sh -c "curl -sfL https://direnv.net/install.sh | bash && cd /tmp && direnv allow . && direnv exec . renovate $argv"
	else
		docker run --rm -it -v (pwd):/tmp renovate/renovate:38.142.4 sh -c "renovate $argv"
	end
    rm -rf renovate/
end

function renovate-config-validator
	docker run --rm -it -v (pwd):/tmp renovate/renovate:38.142.4 sh -c "cd /tmp && renovate-config-validator"
end
