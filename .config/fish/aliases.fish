...
# go required
function aqua-setup
	curl -sSfL https://raw.githubusercontent.com/aquaproj/aqua-installer/v3.1.0/aqua-installer | bash -s -- -v v2.40.0
end
...
function renovate
	if ls ./.envrc
		docker run --rm -it -v (pwd):/usr/src/app renovate/renovate:39.80.0 sh -c "curl -sfL https://direnv.net/install.sh | bash && direnv allow . && direnv exec . renovate $argv"
	else
		docker run --rm -it -v (pwd):/usr/src/app renovate/renovate:39.80.0 sh -c "renovate $argv"
	end
    rm -rf renovate/
end

function renovate-config-validator
	docker run --rm -it -v (pwd):/usr/src/app renovate/renovate:39.80.0 sh -c "renovate-config-validator"
end
